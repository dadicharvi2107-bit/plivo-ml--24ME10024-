"""Optimized trainer. Implements warmup + cosine decay, weight decay splitting, and AdamW tuning.

    python train_opt.py --data ../data/train_corpus.txt --steps 2000 --out ckpt.pt
"""
import argparse
import time
import math
...
def get_optimizer_with_decay_split(model, lr, weight_decay=0.1):
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        # Biases and 1D parameters (norms, embedding weights) do not get weight decay
...
def main():
    ...
    ap.add_argument("--lr", type=float, default=6e-4) # slightly higher default LR for small model with warmup
